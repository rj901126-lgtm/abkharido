...
export default function SellerDashboard({ onNavigate }) {
  ...
  const [activeTab, setActiveTab] = useState('overview'); // 'overview' | 'products' | 'orders' | 'share' | 'payouts'
  ...
  // Load stats and seller products
  useEffect(() => {
    if (currentSeller) {
      fetchSellerStats();
      fetchSellerProducts();
  ...
  return (
    <div className="container animate-fade-in" style={{ maxWidth: '1280px', margin: '20px auto 120px auto', padding: '0 16px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '16px', marginBottom: '24px' }}>
        ...
        <div style={{ background: '#ffffff', borderRadius: '18px', padding: '20px', border: '1px solid #e2e8f0', boxShadow: '0 4px 16px rgba(15, 23, 42, 0.04)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
            <span style={{ fontSize: '12px', fontWeight: '700', color: '#64748b', textTransform: 'uppercase' }}>Total Orders</span>
          ...
          <div style={{ fontSize: '11.5px', color: '#64748b', fontWeight: '600', marginTop: '4px' }}>
            {stats.unitsSold || 0} Units delivered
          </div>
        </div>
        ...
        <div style={{ background: '#ffffff', borderRadius: '18px', padding: '20px', border: '1px solid #e2e8f0', boxShadow: '0 4px 16px rgba(15, 23, 42, 0.04)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
            <span style={{ fontSize: '12px', fontWeight: '700', color: '#64748b', textTransform: 'uppercase' }}>Wallet / Settlement</span>
      ...
      <div style={{ display: 'flex', gap: '10px', borderBottom: '1px solid #e2e8f0', paddingBottom: '12px', marginBottom: '24px', overflowX: 'auto' }}>
        {[
          ...
          { id: 'fulfillment', label: '🚚 NimbusPost Fulfillment' }
        ].map(tab => (
          <button
            key={tab.id}
        ...
        <div style={{ background: '#ffffff', borderRadius: '20px', border: '1px solid #e2e8f0', overflow: 'hidden', boxShadow: '0 4px 20px rgba(15, 23, 42, 0.04)' }}>
          <div style={{ padding: '18px 24px', borderBottom: '1px solid #f1f5f9', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <h3 style={{ margin: 0, fontSize: '16px', fontWeight: '800', color: '#0f172a' }}>
              Your Active Store Catalog ({sellerProducts.length})
            </h3>
            <span style={{ fontSize: '12px', color: '#64748b' }}>
              Click WhatsApp icon to share any item with instant attribution
            </span>
          </div>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '13px' }}>
              <thead>
                <tr style={{ background: '#f8fafc', color: '#475569', borderBottom: '1px solid #e2e8f0' }}>
                  ...
                  <th style={{ padding: '12px 20px', fontWeight: '800' }}>Stock Status</th>
                  <th style={{ padding: '12px 20px', fontWeight: '800', textAlign: 'right' }}>1-Shot Share & Actions</th>
                </tr>
              </thead>
              <tbody>
      ...
      {/* ─── TAB 3: NIMBUSPOST COURIER FULFILLMENT ────────────────── */}
