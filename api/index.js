...
// POST /api/admin/upload-image - Upload generic images like product photos (Admin/Seller)
app.post('/api/admin/upload-image', verifyAdminOrSellerToken, async (req, res) => {
  try {
    const { base64Data, fileName } = req.body;
    if (!base64Data || !fileName) {
      return res.status(400).json({ error: 'Missing image data or filename' });
    ...
    const cleanFileName = `img-${Date.now()}-${fileName.replace(/\s+/g, '-').replace(/[^\w\.-]/g, '')}`;
    const publicDir = path.join(__dirname, '..', 'public', 'uploads');
