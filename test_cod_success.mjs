...
async function testCODSuccess() {
  ...
  const placeOrderBtn = page.locator('button.checkout-btn').first();
  await placeOrderBtn.click();
  await page.waitForTimeout(4000);

  // 7. Capture Success Screen
  await page.screenshot({ path: path.join(artifactsDir, 'step8_order_success_cod.png'), fullPage: false });
